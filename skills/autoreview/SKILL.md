...
The default engine is Codex (model and reasoning effort come from
`~/.codex/config.toml`; currently `gpt-5.6-sol` at high effort). Because the
agent doing the work is usually Claude, a Codex review is a genuine second
model, not an echo — keep it as the default closeout engine. `--engine claude`
(or `AUTOREVIEW_ENGINE=claude`) is fully supported when you want
Claude-on-Claude or Codex is unavailable.

## Privacy and Execution Boundary

The helper sends the frozen git bundle, prompt context, and any `--dataset`
files to the selected review engine. Do not run an external-engine review on
private workspaces unless the user and org policy allow that bundle to leave
the machine or repo boundary. The helper also fails closed before engine
invocation when the bundle contains sensitive-looking paths or secret-like
patch text — redact or split the change instead of bypassing that gate.
