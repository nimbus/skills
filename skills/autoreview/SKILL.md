---
name: autoreview
description: "Run a structured second-model code review (Codex default; optional Claude or Pi) as a closeout check on a local, branch, or commit diff across any Nimbus repo before commit or ship."
---
...
Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `high` reasoning by default, then retries once with `gpt-5.6-terra` only when the account cannot access Sol. Claude review is optional and uses `claude-fable-5` by default. (The nested `codex exec` runs with `--ignore-user-config`, so `~/.codex/config.toml` does not apply inside reviews — the helper defaults above are authoritative.)

## Nimbus adaptations

- **Bundled review criteria**: every review automatically prepends the Nimbus
  repo invariants from `review-context/nimbus.md` plus any repo-local
  additions under `.agents/autoreview/*.md` (alphabetical). Disable with
  `--no-default-context` or `AUTOREVIEW_NO_DEFAULT_CONTEXT=1`.
- **Nested-session guard**: the helper detects Codex-managed sessions
  (`CODEX_SANDBOX` / `CODEX_THREAD_ID`) and refuses nested `--engine codex`
  with a diagnostic — runtime enforcement of the no-nesting contract below.
  `AUTOREVIEW_ALLOW_NESTED_CODEX=1` is the escape hatch for environments
  where nesting is known to work. Inside a Codex job, do a manual
  repo-grounded audit instead; the orchestrating session owns the
  structured second-model review.
- **`/code-review ultra` complement**: this is the local, single-engine
  closeout. `/code-review ultra` is the billed, user-triggered multi-agent
  cloud review — never invoke it on the user's behalf.
...
- Review bundles fail closed before engine invocation when tracked or untracked paths look sensitive or patch text looks secret-like. Obvious synthetic values tied to the credential field, such as `token: "test-token"`, and short `fixture-<service>` labels remain reviewable without one-off allowlists. Safe large diffs are scanned in full, sent as one pass while they fit the aggregate prompt limit, then partitioned into complete bounded passes without truncation.
...
- scans safe Git patches in full, recognizes synthetic values tied to their credential field plus short `fixture-<service>` labels, reviews them in one pass up to the aggregate prompt limit, and automatically uses complete bounded passes above it
